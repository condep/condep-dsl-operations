...
namespace ConDep.Dsl.Operations.Application.Execution.PowerShell
{
    public class PowerShellOperation : RemoteCompositeOperation
    {
        ...
        public override void Configure(IOfferRemoteComposition server)
        {
            ...
            if(RequireRemoteLib)
            {
                server.Deploy.File(Path.Combine(Path.GetDirectoryName(GetType().Assembly.Location), "ConDep.Dsl.Remote.Helpers.dll"), @"%temp%\ConDep.Remote.dll");
                libImport = "Add-Type -Path \"" + @"%temp%\ConDep.Dsl.Remote.Helpers.dll" + "\";";
            }
        ...
        public override bool IsValid(Notification notification)
        {
            ...
            //if(RequireRemoteLib)
            //{
            //    remoteLibExist = File.Exists(Path.Combine(Path.GetDirectoryName(GetType().Assembly.Location), "ConDep.Dsl.Remote.Helpers.dll"));
            //}
