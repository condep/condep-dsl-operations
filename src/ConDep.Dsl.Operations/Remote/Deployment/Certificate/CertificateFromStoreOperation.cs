        ...
        public override void Configure(IOfferRemoteComposition server)
        {
            ...
            try
            {
                ...
                if (cert.HasPrivateKey)
                {
                    ...
                    server.Execute.PowerShell("$path='" + destPath + "'; $password='" + password + "'; $privateKeyUsers = " + psUserArray + "; [ConDep.Dsl.Remote.Helpers.CertificateInstaller]::InstallPfx($path, $password, $privateKeyUsers);", opt => opt.RequireRemoteLib());
                }
                else
                {
